...
| Semantic envelope at `5af693f` | **The whole event family is wired, and sixteen rows with it**: the nine task rows (`e0d097e`, `a7bf33e`, `c81bc55`, `99c13f5`), the five Schedule verbs through one sequence (`4b241f8`), and the recurrence writes in their own module (`5af693f`) - the rule verbs plus the two scopes, named `event.occurrence.change` and `event.series.change` so a journal reader can tell which one a selection meant. Each action mints the id at its own boundary, returns it on every result, and appends exactly one terminal event after convergence through the shell's sink. Four shapes were decided by the work rather than assumed: the drop wrapper mints once and hands its id down so its own refusals are journalled and one drop still leaves one event; a bulk run is one operation whose event's outcome comes from its members; a create names **no** target on a refusal, because it has no id yet; and a plan that can refuse is a **step inside** the run rather than a pre-check in front of it, which is how three recurrence refusals stopped being uncorrelated. Still gaps: the project, workflow-stage and schema families, and the operation-only rows' absent callers. Still gaps: the two recurrence rows (`eventRecurrenceActions.ts`) and the project, workflow-stage and schema families. The contract itself is unchanged since `ac6115a`: `src/app/semanticAudit.ts` holds the outcome vocabulary with **partial** as its own outcome, the one bounded event, the sink, and the single rule that decides the outcome; each action mints its id at its own boundary; and `ProximaActionDispatcher.auditSemantic` is the sink every shell composition supplies, stamping the dispatcher's real state revision rather than one the record layer invented. |
