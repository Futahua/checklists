...
| Awaiting the creator at `e315470` | **44 open boxes, and the line that used to say "two are automatable" now says zero, because one of them was examined and the other was closed.** 37 need the creator at the machine or their data (sixteen native creator-vault acceptances, the clean-profile and real-grant proofs, nine Obsidian-coexistence boxes, the release-readiness revalidations, Gate 20's native grant-registry half, the Obsidian round-trip and concurrent-edit boxes, and Gate 11.3 - a *picked* Windows directory needs one human gesture, which no automated run can produce). 3 are a decision that is theirs: Gate 14's write-enrollment sign-off, the explicit decision to allow writes, and one smallest record type - the gate is built and refuses by default (D83) and nothing in this repository can enroll it. 4 need a Papers change this work may not make: the native-source `grant`/`open`/`reveal` handlers exist in Papers' production preload and not the developer-control preload the automated runs use, so a page's request is answered with silence. Nothing in this repository is left to implement. |
