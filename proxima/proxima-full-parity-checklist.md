...
**Stage 12 has no open box this tree can close** as of `38ce017`: the Event editor is a form when a
record write path has resolved and a projection when it has not, so Save runs `saveEventAction` (the
difference between the record and what was typed, `span` included) and Delete runs `deleteEventAction`,
both at the revision the surface was rendering and both drawing their answer on the form rather than
replacing it — the draft rule D58 asked for, held by the shell. The projection surfaces draw the same
editor, and the case that closes the acceptance box edits two identical worlds through Day and through
Month and compares the resulting records field by field. The recurrence controls stay inert on purpose:
what an occurrence-scoped edit means is Stage 13's question (D59's amendment), and a form that
submitted a half-answered rule would be inventing the answer. Three boxes tick with this slice
(`Save → event.update`, `Delete → event.delete`, `Month/Agenda editor writes same record`).
Next operation: **slice 72 — Stage 13, recurrence and occurrence-scope write parity**, whose first
question is the one this slice deliberately refused to guess at: what "this occurrence" means for a
series, and which of the two write shapes — a series edit or an occurrence override — each scope
selects. Nothing in
