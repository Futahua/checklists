...
| | |
| --- | --- |
| Accepted branch | `stage7-record-store-contract` — creator-accepted through Stage 8 slice 18 at `97c9dd9`; slices 19–45 are pushed at `bd64a34`, `394179c`, `c62a7dc`, `d7e6a6c`, `d66622f`, `a31c74c`, `9b59d16`, `bdea4a1`, `7ec8d17`, `7825d20`, `e88e193`, `b20cdca`, `fef3b8a`, `d7e6270`, `9d6062c`, `1ffdd55`, `d21f434`, `8cadd24`, `3fa16bc`, `ba50cc6`, `7f96a71`, `ead7927`, `215777a`, `08e505d`, `8dc3841`, `d9d8c5e`, `738bb53` and `06c0702` and **await acceptance** |
| Accepted host Gate 9.3 | `Futahua/Papers-3` branch `proxima-gate9-native-source-handoff` @ `67b7fa2` — pushed |
| Accepted host Gate 10.1 | `Futahua/Papers-3` branch `gate10-native-presentation-reconcile` @ `5451bbf` — pushed, creator-accepted |
| Accepted host Gate 10.2 | `Futahua/Papers-3` branch `gate10-host-truth` @ `9e6304b` — pushed, creator-accepted |
| Accepted host Gate 10.3 | `Futahua/Papers-3` branch `gate10-relay` @ `d2a3c74` — pushed, creator-accepted |
| Unaccepted work | none |
| Suite at `06c0702` | fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under **default parallelism**, 183 files / 1225 tests; committer `2026-09-12T02:53:45+07:00`. At `738bb53` the same steps were 182 files / 1220 tests, at `d9d8c5e` 181 / 1214, at `8dc3841` 180 / 1210, at `08e505d` 179 / 1207, at `215777a` 179 / 1205, at `ead7927` 179 / 1203, at `7f96a71` 179 / 1192, and at `ba50cc6` 178 / 1185, `3fa16bc` 178 / 1184, `8cadd24` 178 / 1172, `d21f434` 178 / 1167, `1ffdd55` 177 / 1147, `7825d20` 174 / 1105. **One test was load-sensitive and was fixed, not tolerated:** `tests/bridgeDisclosure.test.ts` starts a real bridge child process and allowed it five seconds to print `listening`; under parallel load that expired while the file passed alone in half a second, which is a flake that makes the whole suite untrustworthy. The bound is now thirty seconds. The slice-30 commit message says "177 files"; that is wrong — two existing files each gained a case, so the file count did not move then. At the last accepted point `97c9dd9`: 168 files / 1011 tests, Stage 8 focused 16 files / 105 tests, committer `2026-09-11T20:42:20+07:00` |
...
refreshes and switches, and a legacy edit or a deletion moves neither the surfaces nor the store — which
leaves item 1's startup/activation half as the only thing between the tree and the write stages.
...
**Next operation** Slice 46 — HARD GATE C item 1's remaining half, which is the last thing before the write
stages: make startup *choose*, and give "successful migration activation" a meaning. Concretely: a candidate built
from a real store backend (the OPFS-backed `RecordStoreFileBackend`, wired the way `externalDirectoryVault` is),
a startup decision that prefers the store once it holds activated records and falls back to the legacy reader when
it does not, and the inspection evidence that says which happened. Items 6 and 7 then follow cheaply — the
"Read-only workspace" label the moment mutations are enabled, and the FSA guard restated for record files. The two
boxes Stage 8 leaves open are still the creator's `unsupported_frontmatter` answer and the tests that encode it.
Nothing in Proxima is parked or uncommitted; the tree is clean and the branch is pushed.
...
- [ ] Startup chooses the Proxima record store as canonical tasks/projects/events/schema source after successful migration activation. *(Half done at `06c0702` @ `2026-09-12T02:53:45+07:00`, and the half that was missing was the hard one: `SourceMode` now has a third member and `createSourceSession` activates a **record-store candidate** — records from the store, notes from the vault — so a store-backed session is a thing that runs, projects and refreshes. What is *not* done is the other half of the sentence: nothing chooses it at startup and there is no "successful migration activation" step to choose it after. `main.ts`'s boot still builds a fixture candidate, and `startupSession` still decides between fixture and external.)*
- [ ] Legacy Markdown record directories remain present but become **legacy source only**. *(Not yet as a property of the shipped product, though both halves are now demonstrable and asserted. Present: the byte-preservation proof (`d9d8c5e`) shows a full staging pass leaves every file byte-identical, and nothing in the tree deletes a legacy record. Legacy source only: with a record-store session active, `tests/recordStoreSourceSession.test.ts` shows an edit to the Markdown those records came from changes nothing the surfaces read, and the vault is read only for notes, drawings and attachments. What is missing is the default: until startup picks the store, the legacy reader is still the record source.)*
...
- [x] Read-only projection/source abstractions are generalized so UI does not care whether state originated from legacy import fixtures or record store. — `738bb53` @ `2026-09-12T02:45:43+07:00` *(the missing piece was that nothing turned `CanonicalRecordV2` into `ProximaState`: the store held canonical records and every surface read the compatibility shape, so a record-store source could not even be represented. `src/app/recordStateProjection.ts` is that projection and `recordStoreStateLoad.ts` is the store-side load, in the same shape as the vault-side one (state plus the revisions a caller needs to notice change). `tests/recordStateProjection.test.ts` proves it end to end — records written through the canonical store boundary, read back through it, projected, and rendered by the real Task Board — and proves the claim that matters: the legacy reader and the record store draw **the same cards in the same three Elastic columns**, so the board asks the state what it holds rather than where it came from. Two things are deliberately not invented: a project's legacy `projectType` is reconstructed from what the project holds (A4 removed it from capability decisions), and a workflow stage is reported as having no slot in the readable world rather than being folded into a column (A2 keeps it independent). The wiring this box left open is done too: `06c0702` gives the session a third mode and a `StateSource` seam, so the two origins are now switchable at runtime, not only in a test's imagination.)*
- [ ] Current UI no longer labels the ordinary product as "Read-only workspace" once record mutations are enabled; at `608bcdc` that label is still hardcoded into the browser shell. *(The condition is not met — mutations are not enabled — so the label is still true. It becomes a defect the moment item 1 lands, and this box is the note that says so. One thing did change at `06c0702`: the header badge no longer calls a record-store session a fixture — it names both halves, "Record store records · vault notes", because a reader who saw only "record store" would reasonably assume the vault was gone.)*
- [ ] Existing FSA creator-vault write boundary remains blocked for record files because record files are no longer creator-vault files at all. *(Half true today and worth stating precisely: the FSA boundary is blocked and asserted (`evaluateFsaWriteBoundary()` fails closed, `tests/fsaWriteBoundary.test.ts`), and record files are already not creator-vault files — they live behind `RecordStoreFileBackend` with opaque file names, and `06c0702` puts a record-store candidate's records behind that boundary while its *reader* is still the vault. The box's "because" arrives with the cutover default, when a record-store file and a vault file could be confused; until then the guard is the boundary itself.)*
- [ ] H4 remains untouched. *(No instruction in this tree defines H4 beyond the creator's standing constraint; nothing in slices 39–45 went near it, and no path here writes to the creator's vault.)*
...
- [ ] Execute semantic JSON-backed task update; every surface changes. *(Not yet: no semantic task update exists to execute — that is Stage 9's, and this gate is what unblocks it. Both halves of the mechanism are now proven separately: a store mutation reaches the projection on the next refresh (`06c0702`), and every surface renders that projection (`738bb53`, `8dc3841`).)*
...
- Cutover integration test. *(Not yet: item 1's startup half and the activation step are what it would integrate. The session-level pieces are proven — a record-store candidate activates, projects, refreshes and switches (`06c0702`).)*
...
- Machine-readable source-mode inspection. — `06c0702` @ `2026-09-12T02:53:45+07:00` *(the third mode is now a value the inspection path can carry: `SourceMode` includes `record-store`, `SourceSessionSnapshot.sourceMode` reports it, `StartupInspection.startupSourceMode` is typed to carry it rather than the old two-member union, and the session case asserts `snapshot().sourceMode === 'record-store'` after activation. Provenance already reports the store's revision per record (`738bb53`). What is still missing is a *shipped* run in that mode, because startup does not select it yet.)*
