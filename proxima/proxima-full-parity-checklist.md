...
| Suite at `c2ef9f1` | fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under **default parallelism**, 220 files / 1456 tests in 11.3 s, re-run 2026-09-12 at this SHA; the 220 is complete coverage rather than a partial collection - 220 `*.test.ts` files exist in the tree and the runner collects all 220, which was checked after the As-you-Go repository turned out to have three test files no script ran |
