...
**Slice 68 opens Stage 12's write half** at `9e20d39`, committed `2026-09-12T05:28:23+07:00`: five event
operations over the store and the recovery gate — `event.create`, `event.update`, `event.delete`, and
the two the checklist names as their own verbs, `event.reschedule` (names the new **start**, takes the
duration from the record) and `event.resize` (names an **end** or a **duration**) — with the dates
deliberately outside `event.update`'s field union, so "keep the duration" is the operation's job
rather than every caller's (D59). `src/app/eventGesture.ts` turns a pointer delta into exactly those
requests: whole fifteen-minute slots, midnight crossed by arithmetic rather than clamped to a column,
and a resize that would leave no duration refused *before* it becomes a request. Five boxes tick —
both agent-parity boxes, the drag's snapping, the resize's snap, and "invalid duration refused before
storage" — and the case that closes "UI geometry converts gesture → same semantic request" hands the
computed request straight to the real operation and compares the record with a direct call's, which is
the only way that claim can mean anything. The remaining Stage 12 boxes are all in the surface: the
empty-cell Save, the editor's Save and Delete, the drag and resize gestures themselves, and the four
acceptance cases. Next operation: **slice 69 — the schedule forms reach these operations**: the
empty-cell seed's Save runs `createEvent`, the Event editor's Save runs `updateEvent`, its Delete runs
`deleteEvent`, all over the same resolved write path the other surfaces use, with the refusals drawn
on the forms. Nothing in
