...
block). **Stage 19, 8** (cross-surface convergence — most of it behind HARD GATE C's shipped trigger).
**Stage 0, 3** (the local-state/equivalence boxes and the revision-in-result box, which its own note gates
on HARD GATE C). **HARD GATE C, 2** — both held by the same missing activation trigger, and H4's box
