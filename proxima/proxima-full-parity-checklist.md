...
**Stage 12's gestures are real writes as of `75091b0` and `da99863`**: the grid's binder hands over an
intent and the shell runs the sequence — a move is `event.reschedule` (the record's duration), a resize
is `event.resize` (the end the pointer landed on), the seeded form's Save is `event.create` — and
`tests/scheduleWriteWiring.test.ts` drives all three into a real store, in Day, 4-Day and Week, with a
lost race asserting the explicit stale loser. Ten boxes tick with them: both empty-cell boxes, four of
the drag's five, three of the resize's five, and two acceptance boxes (the third, Month/Agenda editor
writes the same record, and the fourth, a resized event in every view, both wait on the Event editor's
Save, which is still the read-only projection). The refusal is view state now on this surface too:
the grid draws it on the block it was about, because a schedule draws one event in several columns.
Next operation: **slice 71 — the Event editor becomes a form**: its fields stop being `readonly`, Save
runs `saveEventAction` (the mutations planned from the record and the form, `span` included) and Delete
runs `deleteEventAction`, with both refusals drawn on the modal — which closes Stage 12's remaining two
UI boxes and the last two acceptance boxes. Nothing in
