...
**Stage 17's task and event halves are complete** as of `38bd4ca` and `81e1ec4`: `tests/actionCoverageAudit.test.ts` enumerates sixteen task actions and asserts, per row, the module that owns the write, the call the surface makes to reach it, the test that exercises it, and — where the row claims one — the UI/agent equivalence case, checked against the parity audit rather than trusted. Twenty-three rows tick with it (sixteen task actions and seven event ones); two rows are declared operation-only — task recurrence and an event rule set/clear — and the audit asserts the *absence* of a caller for each, so those gaps are checkable rather than remembered. **The projects half is complete too, at `bede835`** — project create, edit, archive, restore and delete are each a row with its module, its caller, its case and, for delete, the typed refusal the creator has still to answer. What remains in Stage 17 is the workflow-stage and property-schema group (no operation exists for stage create, rename, delete/remap, schema create/update/delete, schema options or field edits, and the audit asserts each absence), the templates row (Stage 16's own work — the shell inside Stage 16 is `src/browser/templateComposer.ts` and the coverage audit waits on the write half), and the eight artifact rows, which wait on the artifact-write question below. Next operation: **slice 80 — the workflow-stage and property-schema write operations**, the last group in this stage whose rows need new code rather than a named question.
...
- [ ] workflow stage create; *(no operation exists, and `tests/actionCoverageAudit.test.ts` proves it by asserting the absence of `createWorkflowStage` in the app layer: a canonical workflow-stage record kind is not a write.)*
- [ ] stage rename; *(likewise, asserted absent as `renameWorkflowStage`.)*
- [ ] stage delete/remap; *(likewise, asserted absent as `deleteWorkflowStage`. Remapping is the harder half: a stage delete has to answer what happens to the tasks in it, which is the same class of question as the project delete policy.)*
- [ ] property schema create/update/delete; *(no operation; asserted absent. Schema records exist canonically and every surface reads them, which is exactly why the distinction between a record kind and a write has to be stated.)*
- [ ] schema options; *(no operation; asserted absent as `updateSchemaOption`.)*
- [ ] formula/rollup/relation schema edits. *(no operation; asserted absent as `updateSchemaField`. A formula is an evaluated field rather than stored data, so writing one is a product question before it is a record write.)*
