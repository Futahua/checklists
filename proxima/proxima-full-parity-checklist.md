...
**Slice 65 is pushed and awaiting acceptance** at `f8376eb`, committed `2026-09-12T04:49:50+07:00`: the Projects
Hub's Archive, Restore and Delete are real controls whenever a record write path resolved and carry the reason
instead when one has not. The distinction is the write path rather than a constant, which is the whole change:
`projectsHub.ts` takes a `ProjectWriteView`, its binder answers `[data-project-lifecycle-action]` before anything
else and ignores a disabled control, and `main.ts` resolves the path through the same `resolveTaskWritePath`
every other surface uses — operations, never storage. Delete stays **enabled** and its `policy-not-decided`
refusal is drawn as a sentence beside it with the member counts (D57), because that refusal is the answer the
creator can act on rather than a feature that is missing. The feedback line is drawn twice, in the controls and
at the list level, because an archived card leaves the active list and would otherwise take the only explanation
of what happened with it. Four UI boxes and one acceptance box tick (`Archive`, `Restore`, `Delete`,
`Project card/hub updates immediately after accepted mutation`, and `UI buttons call the same actions` with
`327d90c`). Stage 11's two remaining UI boxes are named rather than implied: **New Project** still reaches
`project.create` through the action dispatcher instead of `createProjectAction`, which is slice 66's work and not
a claim this slice can make, and **`Edit project fields`** needs a decision this slice deliberately did not
smuggle in — the update operation exists and no form for it does, so adding one is UI rather than wiring. Nothing in
