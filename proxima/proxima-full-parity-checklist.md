...
| Stage 17 matrix at `a983b9a` | The ten columns of *For every row above*, per row, as an inventory in `tests/actionCoverageAudit.test.ts`: each cell is satisfied, a gap, or not applicable **with its reason**, and a gap is asserted as an absence with a witness beside it, so the day the gap closes the audit fails and the cell is revisited instead of staying a gap nobody reads. The AUTHOR's instruction on 2026-09-12 was to build the matrix first and **fix nothing while building it**, because the matrix exists to expose the common gaps. **Templates row recorded**: five satisfied - typed request, typed success, typed validation refusal, affected entity ids, and state/revision observable through convergence and read-back - and five gaps - canonical-ID validation of a submitted `projectId` (only a string check then a cast), semantic conflict and storage failure (both collapsed into `creation-refused`), no system-generated request id on either outcome branch, and no semantic event. Two not-applicable notes live inside those cells: stale is N/A for a create-only run, and recovery is N/A on this path because a create goes through `createIfAbsent` and never journals. |
| Next operation | **Checked and closed at `a983b9a` @ `2026-09-12T10:44:59+07:00`** - the AUTHOR's verdict on the scoped slice was "both boxes can now be ticked", and it confirmed both behaviour changes that came with it (the source refresh now runs after the write rather than racing it, and a click with no usable write path reports a typed refusal instead of being swallowed by the click chain's `!appState` guard). Next: **the matrix, row by row** - Templates is recorded above and the task, event, project, workflow and schema rows follow, in the same table and with the same gap-as-absence rule. Immediately after the matrix has exposed its exact coverage, **one cross-cutting slice, scoped by the AUTHOR on 2026-09-12 as a semantic request envelope plus a semantic terminal event sink - not a redesign of `recordMutation.ts`**. It mints **a separate semantic request id at the public semantic-action boundary**, before validation and before the write path is resolved, and returns it on every success, refusal and partial result, because the coordinator's id cannot serve that purpose: it exists only once an update or delete reaches the coordinator, while the semantic id must also cover refusals decided before storage, creates that go through `createIfAbsent`, a `writes-unavailable` outcome, and one `template.execute` spanning several child creations - and reusing one coordinator id for a batch would give several underlying mutations the same recovery identity. The record-layer contract is left alone, internal child writes keep their own coordinator ids, and **no external caller may supply the semantic id** (the wire gets no requestId; trusted internal layers may pass the already-minted value down). The event half reuses the bounded `EventRing` and its existing action/entity/request correlation vocabulary, but preserves its categories: a clean acceptance is `action.accepted`/`domain` and a zero-write refusal is `action.rejected`/`diagnostic`, while **partial needs explicit representation** - `action.partial` or an equivalent explicit outcome field carrying every landed entity id plus the refusal code, because "one record landed and the second failed" is neither an acceptance nor a rejection. The event is appended **through an injected audit sink after convergence**, where composition can supply the real observable state revision; the record layer must not fabricate one, and if no real revision is available that matrix cell stays a gap rather than using zero. The AUTHOR was explicit that those two boxes are not to wait for an unrelated future audit. Then the ten non-test matrix boxes, whose meaning the AUTHOR has now fixed column by column: typed request, typed success and validation refusal may be asserted structurally, while runtime validation, stale/conflict, storage or recovery failure, post-state and the audit/event record need behavioural evidence; N/A is allowed only for a capability the creator has decided not to offer (D63 artifact writes) or for success semantics on a refusal-only verb, never because infrastructure is missing. The AUTHOR's reading of the Templates row at `4d0bfaa`: typed request, typed success, validation refusal, created ids and post-write convergence exist, while conflict and storage failure are currently collapsed behind `creation-refused`, there is no top-level request id, and there is no semantic event - so five of the ten close as they stand and five need work. The eight open work boxes inside the agent-protocol and action sections are the slice after that; the two recurrence rows stay deliberately open as gaps the audit asserts, which the AUTHOR confirmed. Earlier scoping, kept because it is what produced the executor: the Stage 16 executor core and preflight - `executeTemplatePlan` takes a typed plan plus an injected `TaskCreateOperations` port and an injected clock, never imports or receives RecordStore, refuses before the first call on an invalid plan or an untranslatable draft field, reports a mid-batch refusal as partial rather than complete, and is covered by tests for zero calls on failed preflight, the exact emitted requests, deterministic clock and IDs, and a partial write reported as partial. |
