...
**Stage 13 has no open box** as of `f3f277a`: the canonical series now reaches the readable world (the
projection was dropping it, so a series could be written and never drawn — a write that appears to work
and reverts), the expansion honours its exceptions, and the scope modal's two scopes carry two genuinely
different writes — one exception keyed by the slot the rule generated, or the owner record's span with
every occurrence following it. An override cannot name an instant the rule does not generate, because
the planner regenerates the occurrence with the same function the expansion uses; the scope is always a
parameter, so a reader's choice and an agent's request produce the same record, which the case asserts
by editing two identical worlds both ways and comparing the resulting series field for field. The
recurrence controls in the Event editor are still inert, and that is now a *stated* boundary rather than
an unfinished one: the editor's recurrence fields are Stage 13's other half and are named there.
Next operation: **slice 74 — Stage 14, Timekeeping/Gantt DATA WRITE parity**, the next stage with open
boxes this tree can close. Nothing in
