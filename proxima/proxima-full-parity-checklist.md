...
| Next operation | **The agent write path landed at `32132bb` @ `2026-09-12T12:01:23+07:00`, so the first record verb an agent can submit exists, and seven boxes closed with it.** `src/app/agentWritePath.ts` is a sibling entry, not a `ProximaAction`: its wire names `task.execution.move`/`task.execution.reorder` plus the facts the agent read (`taskId`, `from`, `to`, `targetIndex`, and a **required** `expectedRevision`), mints one request id before it parses anything so even a malformed submission is correlatable, and hands the work to `moveTaskByGesture` - the same operation the Elastic drop wrapper runs, which is why `performElasticDrop` stopped narrowing its result: a parity claim compared against a subset of fields can hide a divergence in the fields one side drops. The dispatcher's containment rule is untouched, and the same suite asserts `dispatch` still refuses the same verb with the store byte-identical, beside the write that succeeds. A verb enters the wire only when its operation runs with no cockpit, so every other registered record or artifact verb is answered `unsupported-verb` with the sentence saying where it belongs, derived from `registeredActionTypes()` rather than a hand-kept list. **The next slice is the property-schema action layer**, which is what the three remaining data-write universals now wait on: the schema family (create, rename and delete a declared property) has no semantic action at all, so there is no verb for the wire to answer, no typed result to carry and no envelope cell to fill - the matrix records it as three structural-gap rows rather than as missing fields. The same reasoning covers the other cockpit-bound families (`task.create` needs a draft and the projection it defaults from, `task.timeline.change` finds its task in the state, and the `project.*`, `event.*`, `workflow.stage.*` and bulk verbs likewise): each becomes agent-reachable when its operation is read-model-free, or when an agent can hold a read model, and until then the wire's refusal sentence is the honest answer. The cells that stay open on purpose remain named: the project delete's affected ids, which it reports as counts, the operation-only rows' absent callers, and the three schema rows' request-id and event cells. The contract is not to be redesigned: `src/app/semanticAudit.ts` (outcome vocabulary including `partial`, the one bounded event, the sink, and the single outcome rule), the id minted at each action's boundary and returned on every branch, the terminal event appended after convergence, and `ProximaActionDispatcher.auditSemantic` as the sink every shell composition supplies. No wire accepts a caller-supplied id, and the record-layer contract stays untouched. |
...
- [ ] Every human DATA WRITE gesture has one semantic Proxima action. *(Its state changed at `32132bb` @ `2026-09-12T12:01:23+07:00` without becoming true: the gesture families are inventoried by the contract matrix (`tests/actionCoverageAudit.test.ts`, 36 rows) and the drop gesture's two verbs now have an agent-facing submission entry, but the property-schema family — create, rename, delete a declared property — is a human data write with no semantic action layer at all, which the matrix records as three rows of structural gap rather than as missing fields.)*
- [ ] Every such action has typed success/refusal/stale/failure. *(Same blocker, stated by the same matrix: the rows that have an action carry the typed verdicts, including the two the agent write path now returns; a family with no action has no result type to type. The store-level verdicts exist and are conformance-tested independently.)*
- [ ] UI and agent paths share implementation. *(Partly true at `32132bb` @ `2026-09-12T12:01:23+07:00`, and the part that is not is named. The drop family is shared: `src/app/agentWritePath.ts` and the Elastic drop wrapper both run `moveTaskByGesture`, and `tests/agentWritePath.test.ts` compares them as whole objects. The remaining families are not, for one reason rather than neglect — their operations read a rendered cockpit (`task.create` needs a draft and the projection it defaults from, `task.timeline.change` finds its task in the state, the `project.*`, `event.*`, `workflow.stage.*` and bulk verbs likewise) — and the wire answers each of those verbs `unsupported-verb` with the sentence saying where it belongs rather than pretending they are unknown. Each family needs its operation made read-model-free, or a read model an agent can hold, before it can be shared.)*
