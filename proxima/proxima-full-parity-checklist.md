...
| Awaiting the creator at `0c80b85` | **The 45 boxes still open in Proxima split four ways, and only three of them are decisions.** 36 need the creator at the machine or their data (sixteen native creator-vault acceptances, the clean-profile and real-grant proofs, nine Obsidian-coexistence boxes, the release-readiness revalidations, Gate 20's native grant-registry half, and the Obsidian round-trip and concurrent-edit boxes). 3 are a decision that is theirs: Gate 14's write-enrollment sign-off, the explicit decision to allow writes, and one smallest record type - the gate is built and refuses by default (`src/app/writeAdmission.ts`, D83), and nothing in this repository can enroll it. 4 need a Papers change this work may not make: the native-source `grant`/`open`/`reveal` handlers exist in Papers' production preload and not in the developer-control preload the automated runs use, so a page's request is answered with silence and those boxes cannot be observed from here. 2 are automatable through the host seam and un-attempted: a browser-native `FileSystemDirectoryHandle` used without a picker, and a real FSA acceptance over disposable Windows directories - both are runs rather than decisions. Proxima's suite is green at the SHA above; this repository is documentation only. |
