...
| Semantic envelope at `2dadbe1` | **Twenty-one rows are wired**: the nine task rows (`e0d097e`, `a7bf33e`, `c81bc55`, `99c13f5`), the seven event rows (`4b241f8` for the five Schedule verbs through one sequence, `5af693f` for the recurrence writes with the scope in the journal's name) and the five project verbs through one sequence (`2dadbe1`). Each action mints the id at its own boundary, returns it on every result, and appends exactly one terminal event after convergence through the shell's sink. Five shapes were decided by the work rather than assumed: the drop wrapper mints once and hands its id down so its own refusals are journalled and one drop still leaves one event; a bulk run is one operation whose event's outcome comes from its members; a create names **no** target on a refusal, because it has no id yet; a plan that can refuse is a **step inside** the run rather than a pre-check in front of it; and a refusal that **is** the answer - the project delete policy question - is journalled and coded like any other, because an answer with no trace is the one path a reader cannot audit. Still gaps: the workflow-stage and schema families, and the two count-summarising cells (the project delete's affected ids, which it reports as counts) that the envelope does not close. | Still gaps: the two recurrence rows (`eventRecurrenceActions.ts`) and the project, workflow-stage and schema families. The contract itself is unchanged since `ac6115a`: `src/app/semanticAudit.ts` holds the outcome vocabulary with **partial** as its own outcome, the one bounded event, the sink, and the single rule that decides the outcome; each action mints its id at its own boundary; and `ProximaActionDispatcher.auditSemantic` is the sink every shell composition supplies, stamping the dispatcher's real state revision rather than one the record layer invented. |
