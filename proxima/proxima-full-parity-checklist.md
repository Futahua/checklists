...
- [ ] Define whether deleting a project: *(**the creator's question, and now the only thing between Stage 11's Delete and Delete doing something.** The three candidates are unchanged — leave members uncategorised, require an explicit cascading action, or refuse while members exist — and `04d0bc4` deliberately implements none of them: `deleteProject` refuses with `policy-not-decided` and names the member counts so the question can be answered from the message. `f8376eb` puts that refusal where a reader clicks, so the question is now asked by the product rather than only by a test. `docs/DECISIONS.md` D56 records the choice and what each possible answer would change; when the creator answers, that one function changes and every caller follows, because there is one operation rather than one per surface.)*
