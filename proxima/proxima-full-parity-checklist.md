...
- [ ] UI invocation test exists; — **reopened 2026-09-12 by the browser AUTHOR, and the route it named is applied at `a983b9a` @ `2026-09-12T10:44:59+07:00`; unticked pending its check of that evidence.** `716739c` @ `2026-09-12T10:26:58+07:00` and `4d0bfaa` @ `2026-09-12T10:29:04+07:00` had produced the two halves this repository could produce - the panel rendered while bound to the shell's own options, and the click chain read as source - and the AUTHOR rejected that as this box's evidence: *"reading main.ts as text cannot prove the listener actually invokes the action under runtime composition."* What exists now: `src/browser/templateExecuteBinding.ts` holds the binding (`bindTemplateExecuteInteractions`, one delegated listener that resolves the control from the event because the panel re-renders under it), `main.ts` calls `bindTemplateExecute(root)` and **no longer has a branch of its own for the verb**, and `tests/templateExecuteClick.test.ts` renders the real composer, binds that module, clicks `data-c1-key="template-execute"` through the existing `InteractionHarness`, and asserts the records in a real store, the panel's reported ids against the ids that landed, the running state and its note while a real write is gated, that a second click mid-run creates nothing extra, that a refusal leaves the composer open with the template, and that a run refused on its second create is reported as partial with the one id that landed. Pointing the listener at a verb the panel does not declare fails all five cases, so the file is not passing vacuously. The EXECUTOR has left the box unticked because it does not accept its own work.
- [ ] equivalence test exists. — **reopened 2026-09-12 by the browser AUTHOR, and the comparison it named is applied at `a983b9a` @ `2026-09-12T10:44:59+07:00`; unticked pending its check.** D71's boundary was ratified - moving the comparison from `executeTemplatePlan` up to `executeTemplateAction` at `4d0bfaa` @ `2026-09-12T10:29:04+07:00` is correct, because it proves record equivalence through the shipped action layer - but that case answers Stage 16's box, not this one: this box sits beside *agent invocation* and *UI invocation*, so its claim is equivalence **between those two callers**. `tests/templateCallerEquivalence.test.ts` now runs the same template through a click and through `submitTemplateExecution` in two isolated stores, and compares what each caller reports (including that the panel's sentence is the action's own refusal detail) and the decoded canonical records with identity and timestamps normalised away, with a control - a longer template through the agent - that proves the comparison can fail.
