...
| | |
| --- | --- |
| Accepted branch | `stage7-record-store-contract` — creator-accepted through Stage 8 slice 18 at `97c9dd9`; slices 19–47 are pushed at `bd64a34`, `394179c`, `c62a7dc`, `d7e6a6c`, `d66622f`, `a31c74c`, `9b59d16`, `bdea4a1`, `7ec8d17`, `7825d20`, `e88e193`, `b20cdca`, `fef3b8a`, `d7e6270`, `9d6062c`, `1ffdd55`, `d21f434`, `8cadd24`, `3fa16bc`, `ba50cc6`, `7f96a71`, `ead7927`, `215777a`, `08e505d`, `8dc3841`, `d9d8c5e`, `738bb53`, `06c0702`, `d6e2b30`, `e898a04`, `abf8204`, `e62e8f4`, `ed09e3d`, `cab1627`, `18c2e48`, `9a04451`, `e4e319b`, `c74003f`, `1029a25`, `6dfad33`, `08d31b5`, `e325f6e`, `9dcc5d6`, `e9117b2`, `9b197e2`, `04d0bc4`, `327d90c`, `f8376eb`, `c5291f1`, `d5fbc00`, `9e20d39`, `392a8e7`, `2e7928d`, `75091b0`, `da99863`, `dc82f11`, `38ce017`, `16b443e`, `f3f277a`, `bd61f60`, `8d5ec96`, `38bd4ca`, `81e1ec4`, `bede835`, `f17ba68`, `9e27cad`, `1b0094a`, `4288b71`, `dc675d9`, `8676dc3` and `425a631` and **await acceptance** |
| Accepted host Gate 9.3 | `Futahua/Papers-3` branch `proxima-gate9-native-source-handoff` @ `67b7fa2` — pushed |
| Accepted host Gate 10.1 | `Futahua/Papers-3` branch `gate10-native-presentation-reconcile` @ `5451bbf` — pushed, creator-accepted |
| Accepted host Gate 10.2 | `Futahua/Papers-3` branch `gate10-host-truth` @ `9e6304b` — pushed, creator-accepted |
| Accepted host Gate 10.3 | `Futahua/Papers-3` branch `gate10-relay` @ `d2a3c74` — pushed, creator-accepted |
| Unaccepted work | none |
| Suite at `c2ef9f1` | fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under **default parallelism**, 223 files / 1468 tests, re-run 2026-09-12 at `c689283` after the Stage 16 executor slices (the earlier figures on this row were 220 / 1456 at `c2ef9f1`); the 220 is complete coverage rather than a partial collection - 220 `*.test.ts` files exist in the tree and the runner collects all 220, which was checked after the As-you-Go repository turned out to have three test files no script ran |
| Next operation | Stage 16 **executor core and preflight**, scoped by the browser AUTHOR on 2026-09-12 - no browser wiring and not `template.execute` yet. The parser half already exists at `d21f434` @ `2026-09-12T01:26:26+07:00` (`parseTemplatePlan` -> typed `TemplatePlan`, structured refusals, no imports at all). Accept the slice only if: **1.** `executeTemplatePlan` takes a typed plan plus an injected semantic-action port, an opaque-ID generator and a clock, and never imports or receives RecordStore; **2.** any parser error rejects the whole execution before the first action; **3.** every ID for the batch is allocated before the first semantic action, opaque, unique, stable for that run, and intra-batch relation references resolve only through that allocation table; **4.** draft fields are translated into the same canonical create/update requests used outside templates, with no template-specific mutation implementation; **5.** a fixed clock makes relative-date resolution deterministic including boundary cases; **6.** results distinguish complete, refused and partial, name the created IDs and the failures, and never report complete after a partial write; **7.** tests prove zero calls on invalid or failed preflight, the exact emitted semantic requests, deterministic clock and IDs, relation correctness, and a mid-batch refusal reported as partial. This closes architecture boxes only - the agent, UI and restart acceptance boxes and Stage 17's Templates row stay open until execution is wired. Two facts already checked so the slice is smaller than it looks: no batch-creation API exists anywhere in the record layer, and the domain already carries `OpaqueRecordId` with `parseOpaqueRecordId` in src/domain/canonicalArtifactAssociation.ts. **The seam, named by the AUTHOR on 2026-09-12 after checking the tree:** the semantic-action port is `TaskCreateOperations` from `src/app/taskCreate.ts` - translate each `TemplateTaskDraft` into a `CreateTaskRequest` from `src/app/taskMutations.ts` and call `createTask`, and do **not** make template drafts the port vocabulary or use `ProximaAction`, because the public union has no `task.create` and no `taskWriteActions.ts` exists at this SHA. Take the `Clock` from `src/domain/clock.ts`, but **not** its generic generator for canonical ids: those are plain strings, and a record id must be the `pxr_<32 hex>` opaque form, so the slice exposes `allocateRecordId()` grounded in `src/domain/canonicalIdentity.ts`. And **no allocation map in this slice**: `TemplateTaskDraft` carries no unique local identifier and duplicate display names are not identity, so a table keyed by name would invent identity - it becomes a Map only when an explicit parser-validated template-local id arrives with cross-record references. **Corrected scope, AUTHOR ruling 2026-09-12:** the next authority-bearing slice is **not** the composer panel. Add the canonical `template.execute` submission path, backed by `executeTemplatePlan`, and then make **both** the agent submission and the panel Confirm call that same path - HARD GATE C no longer blocks writes (the checklist says the write stages are no longer gated), Stage 16 requires agent execution without a modal, and Stage 17 wants UI/agent equivalence. When the panel does come, its Confirm action must treat a `partial` result as unfinished: keep the composer open, preserve the plan and the input, report the created ids and the failures visibly, and offer retry or correction - never close or reset as though execution had completed. **Landed 2026-09-12 at `314b7d1` @ `2026-09-12T10:04:56+07:00`:** src/app/templateExecuteAction.ts is that submission path - `executeTemplateAction(deps, { template, projectId? })` parses the text itself so a caller cannot execute a hand-built plan, resolves the operations structurally through the shell (`writes()`, no store or coordinator reachable from the action), and answers `complete` with the created ids or `ok: false` with a reason and whatever ids did land. Writing its tests found a semantics bug in my own mapping: `accepted` was passed as "the whole run completed", so a partial run told convergence that nothing had changed and the surfaces were left stale after records had in fact been created - convergence is now owed whenever the store changed, while a refusal decided before the first call still does not re-read. Six tests, `npm test` 224 files / 1474 tests, typecheck 0. What remains of the AUTHOR scope: the agent submission and the panel Confirm both calling this path, with the panel treating `partial` as unfinished. **Agent entry landed 2026-09-12 at `028316e` @ `2026-09-12T10:07:27+07:00`:** src/app/templateSubmission.ts holds the agent-facing wire shape (`{ type: 'template.execute', template, projectId? }`) and validates **only** that outer shape before calling `executeTemplateAction` - the same path the panel Confirm will call, so an agent and a surface cannot drift apart. The AUTHOR ruled it a **sibling entry rather than a ProximaAction**, because `dispatch()` is synchronous and generates its own requestId, and forcing an asynchronous multi-record operation through it would change the execution model of every unrelated action; **Stage 17 should register this sibling as the Templates agent submission path rather than pretending it passed through the union.** No caller-supplied requestId. Four tests, including the one the AUTHOR named: submit with no panel or modal in the dependency set and prove the created records carry the returned ids. `npm test` 225 files / 1478 tests, typecheck 0. What remains: the panel Confirm calling the same path, treating `partial` as unfinished. **Surveyed for the next session rather than guessed:** the panel is `renderTemplateComposerPanel` in src/browser/templateComposerPanel.ts - a pure string renderer with no handlers, whose Execute button is rendered **disabled** with `data-template-execute-refusal="action-not-available"` and the sentence *"Execution unavailable until task records can be written. Nothing has been created."* (`TEMPLATE_EXECUTE_NOTE`). Its only call site is src/browser/projectBacklog.ts:83, which passes `{ text, open, active }`, and I wrote last round that the click path is that file's `data-project-backlog-action` handler - **that was wrong, and checking it found the real shape of the slice**: no file in this repository, in `Papers-3`, or in `Products\Papers\Source` reads that attribute at all, so the backlog's action dispatch **does not exist yet** and writing it is part of the remaining work rather than a place to hang one case. The shell that builds write dependencies for surfaces is `src/browser/main.ts` (1,694 lines), and the next probe made the remaining work mechanical: **the shell has exactly one live click convention** - a delegated listener at main.ts:1640 matching `[data-action]`, an if/else on `dataset.action`, and `dispatchAction({ type })` - while the backlog's `data-project-backlog-action` attributes are a different, unread convention. And **`TaskCreateDependencies` (src/app/taskCreate.ts:258) is structurally identical to `TemplateExecuteDependencies`** - same six fields - with `TaskCreateOperations` being precisely the template port, so the whole shell call is `executeTemplateAction(taskCreateDependencies(), { template })`, and `taskCreateDependencies()` at main.ts:761 already supplies `state`, `writes: resolveTaskWritePath`, `unavailableReason`, `refresh: refreshFromSource`, `setRefusal`, `render`. What remains is therefore small and specific: render the Execute button **enabled** with `data-action="template-execute"` when the plan is clean and the view is active, add the branch to that click chain, hold the result in view state so the panel can show it, and keep the composer open on a `partial` result with the created ids and the failure shown. So the slice is: add the Execute action there, build `TemplateExecuteDependencies` from the shell (state, `writes()`, `unavailableReason`, `refresh`, `setRefusal`, `render`), call `submitTemplateExecution`, and give the renderer the three states it lacks - enabled when the plan is clean and the view is active, running, and a result that **keeps the composer open** with the created ids and the failure shown. Until that lands the disabled button is the honest UI, which is why it was left alone rather than half-enabled. **Renderer landed 2026-09-12 at `d5c7622` @ `2026-09-12T10:12:20+07:00`:** the panel now draws all three states - enabled when the plan is clean and the view is active, waiting while a submission is in flight, and a result block naming the created ids and the failure while leaving the template in the box - and a result never closes the panel, so a partial run reports how many tasks were made before it stopped and keeps the composer usable. The safety property is that `executable` defaults to **false**: teaching the renderer to draw an enabled button does not enable it anywhere, the shell turns it on in the same change that adds the click branch, and the first test asserts that default so a later pass cannot make the button live by forgetting an option. Seven tests, `npm test` 226 files / 1485 tests, typecheck 0. **The shell half landed 2026-09-12 at `694fb6a` @ `2026-09-12T10:16:46+07:00`:** the backlog view carries `templateExecuting` and `templateResult` and passes `executable`/`executing`/`result` to the panel; `createTemplateTasksAction()` sits beside the existing template handlers and calls `executeTemplateAction(taskCreateDependencies(), { template })`, storing the outcome in view state; the click chain has its `template-execute` branch; and the test that asserted *execution is refused with action-not-available* now asserts the button is offered and wired while keeping its own invariant that opening a panel leaves the store byte-identical. The composer stays open with its result rather than closing over it. **What has no coverage, deliberately:** the click chain itself - the backlog test harness binds its own interactions and never runs the shell chain, so clicking `template-execute` there cannot reach the action; that test was written, seen to fail for that reason rather than a bug, and removed rather than left red or faked. What remains for Stage 16 is Stage 17's registration of the sibling submission entry and the agent/UI/restart acceptance boxes. **The shell half is now four located edits, and one of them is a test that must change with the behaviour rather than be worked around:** (1) src/browser/projectBacklog.ts - carry `templateResult` and `templateExecuting` in `ProjectBacklogViewState` (lines 22-23) and pass `executable: true` plus those fields to the panel at line 83; (2) src/browser/main.ts - a `createTemplateTasksAction()` near the existing `openTemplate`, `closeTemplate` and `setTemplateText` handlers (lines 1457-1466) that calls `executeTemplateAction(taskCreateDependencies(), { template })` and stores the result in the view; (3) the main.ts click chain - a `template-execute` branch inside it, and that chain closes at line 1714 after the `source-refresh` branch at 1696-1700; (4) tests/projectBacklog.test.ts:404 - the test named *refuses execution with a typed result rather than hiding the control* asserts `execute.disabled === true` and a refusal of `action-not-available`, which is exactly the truth this slice replaces, so it becomes an assertion that the button is offered (enabled, carrying `data-action="template-execute"`, no refusal attribute) while **keeping its own invariant** that opening a panel leaves the store byte-identical. **Verification notes a next session should not have to rediscover:** (1) `npm test` alone is not enough - it has passed while `npm run typecheck` failed, three times in one session, every time on a test-only slip such as `expect(value, 'message')`, which vitest rejects; both must exit 0 before a commit. (2) `tests/bridgeDisclosure.test.ts` has a 30-second timing bound that flakes under full-suite load; it passes in isolation in about half a second, so a red suite naming that file is the flake rather than a regression. (3) The tick-reference audit over both checklists - every ticked box resolving to a real commit with its exact committer timestamp - was run and passed at 786 references; re-run it after any pass that ticks boxes by hand. (4) Proxima has four deliberate exceptions in that audit nowhere (all ticks carry commits), while Quick Run has exactly two ticks without a commit reference, the Sets-decision and hotkey-chord boxes, whose evidence is a creator decision inline by date. (5) The two window checklists are accurately documented as machine-gated; do not re-litigate that. **Templates row closed 2026-09-12 at `7ae11f2` @ `2026-09-12T10:24:35+07:00`:** Stage 17's `execute.` for the Templates row is ticked with its evidence named per concern, and the registration the AUTHOR asked for is in the tick - Stage 17 points at `src/app/templateSubmission.ts` as the Templates **agent** submission path rather than pretending it passed through `ProximaAction`. What is left in Stage 17 is the *for every row above* matrix, and on the Templates row specifically it is blocked by one thing: **a UI invocation test**. The backlog harness binds its own interactions and never runs the shell click chain, so there is no harness on this machine that can click `template-execute` and observe the action - the test was written, seen to fail for that reason rather than a bug, and removed rather than left red or faked. A session that wants that box needs either a shell-level harness or a decision that the chain is covered by construction.
...
- [ ] workflow stage where project-scoped; *(The separation exists in the domain: HARD GATE A2 is accepted at `07d4926`, which is an ancestor of this branch, and `src/domain/canonicalRecordV2.ts`/`canonicalOrdering.ts` carry `executionState` and `workflowStageId` as independent dimensions. The Task modal edits the legacy compatibility `Task` in `src/domain/types.ts` — whose `status` field is commented "Not the future canonical execution/workflow model" — because the reader still loads legacy Markdown. The box closes when canonical records reach the modal, which is the record-store read path, not a modal change.)* **Checked rather than assumed:** `src/app/taskEditor.ts` carries no workflow field and neither the editor nor the cockpit suite names one — the workflow dimension is owned by the project workflow board, where stages are columns and a drop is the write. So this box's condition is *half* met: canonical records do reach the modal (the record-store read path projects `workflowStageId`/`workflowOrder` into the same `Task` the editor reads), but the modal deliberately has no field for it. Closing the box is therefore a small **product decision before it is code**: does the Task modal show a workflow-stage control — which then has to say *where* in the stage the card lands, the same position a drop asks for, or append to the end — or does the board stay the only place the workflow is edited? If the answer is "the board only", this box should be struck rather than left open, and that is the creator's call, not an implementation detail.
...
- [ ] task recurrence if retained; *(**declared operation-only, and the audit asserts that**: the record layer accepts a `recurrence` series for a task — the mutation kind is asserted — and no surface reaches it, which `tests/actionCoverageAudit.test.ts` checks by asserting the absence of a caller in the shell. The row is ticked when a UI caller exists, not when the operation does.)*
...
- [ ] recurrence set/clear; *(**declared operation-only, and the audit asserts that**: `setRecurrenceAction` and `clearRecurrenceAction` write a canonical series over the store and the recovery gate, the projection now carries it into the readable world, and the Event editor's recurrence controls stay deliberately inert — what an occurrence-scoped edit means is settled, and what a *rule* edit should look like in the form is not yet offered. The row is ticked when a UI caller exists.)*
