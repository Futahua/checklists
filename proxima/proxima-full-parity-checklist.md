...
**Stage 14's write is real as of `bd61f60`**: a bar drag or an edge resize reaches
`src/app/timelineChangeAction.ts` instead of the dispatcher, so the gesture's two proposed dates become
one `dates` mutation at the revision the bar was drawn from, an inverted or unreal range is refused
before storage with its own sentence, and a lost race re-reads — which is what restores the
authoritative bar rather than the one the pointer left. The row the bar landed in is *reported*
(`rowApplied: false`) and deliberately not written, because A3 settled that Gantt row placement is local
rather than a third durable task order; the case moves a bar five rows and asserts the record's
execution order, workflow stage, workflow order and properties are all unchanged. Four boxes tick with
it (both refusal boxes, the agent's coordinate-free request, and the scoped-row guarantee). What is left
in Stage 14 needs a case that drives the *gesture itself* into a real store — the shape
`tests/scheduleWriteWiring.test.ts` took for Stage 12 — which is what closes the four remaining gesture
boxes and the two acceptance boxes that compare the UI's dates with an agent's and check that the
Countdowns and the Deadline Calendar follow. Next operation: **slice 75 — the Gantt gesture case**:
pointerdown on a bar, move, release, through the real binder into a real store. Nothing in
