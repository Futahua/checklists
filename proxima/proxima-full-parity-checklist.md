...
**Stage 17's task half is complete** as of `38bd4ca`: `tests/actionCoverageAudit.test.ts` enumerates sixteen task actions and asserts, per row, the module that owns the write, the call the surface makes to reach it, the test that exercises it, and — where the row claims one — the UI/agent equivalence case, checked against the parity audit rather than trusted. Sixteen rows tick with it; task recurrence is the one row declared operation-only, and the audit asserts the *absence* of a caller for it, so that gap is checkable rather than remembered. The other Stage 17 groups (projects/workflow/schema, events, artifacts, templates) are next, and the artifact group waits on the artifact-write question below.
