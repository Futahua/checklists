...
| Stage 17 matrix at `f9c202a` | **Complete for every row the agenda carries**: 36 rows / 360 cells - 248 satisfied, 111 gaps, one not applicable at that revision, and **nine of those gaps closed at `65f88ef`** (the three schema rows' boundary, request-id and event cells, which now read `src/app/propertySchemaActions.ts` while the record layer's own cells keep asserting their gaps), so 257 satisfied / 102 gaps when read at the current tree - in `tests/actionCoverageAudit.test.ts`, one table per family (task, event, project, workflow-stage, schema) with each family's shared cells written once and each row's request, refusal, ids and observable cells written for that row. Each cell is satisfied, a gap, or not applicable **with its reason**; a gap is asserted as an absence with a witness beside it and its anchor asserted before it scopes anything, so a gap fails the day it closes and no check can pass by slicing the last character of a file it never read. Five walkers assert the ten columns in the AUTHOR's order and that each table covers exactly the rows the file already carries. What remains is not scattered: 35 rows still have no untrusted boundary accepting `unknown` (the only one in the tree is the template submission entry), 35 still carry no semantic request id, 35 still emit no semantic event, the retained task recurrence is unwired **and** unobserved, the schema field edit is asserted on the record the operation returned and never read back, and `project.delete` answers with counts rather than the ids it would affect. **Templates row after `ac6115a`**: seven satisfied, three gaps, with stale N/A for a create-only run and recovery N/A because `createIfAbsent` never journals. |
| Next operation | **The agent write path (`32132bb`) and the property-schema action layer (`65f88ef`) are both in, so the three schema rows now carry a boundary, a request id and an event, and one of the three data-write universals is ticked.** Twenty-five boxes are left, and they fall into four groups, named here so the next slice is chosen rather than guessed. **(1) The agent reach of the remaining families** - largest, and the one HARD GATE E keeps asking about. The drop family is shared between the Elastic wrapper and `src/app/agentWritePath.ts`; every other family's operation reads a rendered cockpit, so each becomes agent-reachable when its operation is made read-model-free or when an agent can hold a read model. **(2) The operation-only rows' callers**: `task.recurrence` and the event recurrence set/clear are wired, audited and declared operation-only; they close when a surface offers them. **(3) The schema family's own caller**: it has an action now and neither a UI nor an agent entry, which is the state the Templates row was in before `a983b9a` - a schema editor, or an agent entry on the same wire, is what closes it. **(4) The boxes that are not about code at all**: the scoped-ordering test that does not exist, panel sizing, the navigator-render extraction behind the project-archive box, the three scope statements this repository cannot evidence alone, and the creator-interaction claim the two machine-gated window checklists contradict for their own scope. The contract is not to be redesigned: `src/app/semanticAudit.ts` (outcome vocabulary including `partial`, the one bounded event, the sink, and the single outcome rule), the id minted at each action's boundary and returned on every branch, the terminal event appended after convergence, and `ProximaActionDispatcher.auditSemantic` as the sink every shell composition supplies. No wire accepts a caller-supplied id, the record-layer contract stays untouched, and a record verb the protocol refuses is reached through a named sibling entry rather than by loosening `action-not-available`. |
...
- [ ] Every such action has typed success/refusal/stale/failure. *(The remaining gaps are per row and named by the matrix rather than general: the Templates row collapses a semantic conflict into `creation-refused`, the schema field-edit row's post-action state is asserted on the value the operation returned rather than read back, and `project.delete` answers with counts where its siblings answer with ids. Each is a cell in `tests/actionCoverageAudit.test.ts` with its own reason; a family-wide "yes" here would contradict them.)*
- [ ] UI and agent paths share implementation. *(Partly true, and the part that is not is named. The drop family is shared — `src/app/agentWritePath.ts` and the Elastic drop wrapper both run `moveTaskByGesture`, compared as whole objects at `32132bb` @ `2026-09-12T12:01:23+07:00`, and the schema family now has an action but neither a UI caller nor an agent entry, so it shares nothing yet (`65f88ef` @ `2026-09-12T12:13:11+07:00`). The remaining families are not shared for one reason rather than neglect — their operations read a rendered cockpit (`task.create` needs a draft and the projection it defaults from, `task.timeline.change` finds its task in the state, the `project.*`, `event.*`, `workflow.stage.*` and bulk verbs likewise) — and the agent wire answers each of those verbs `unsupported-verb` with the sentence saying where it belongs. Each family needs its operation made read-model-free, or a read model an agent can hold, before it can be shared.)*
