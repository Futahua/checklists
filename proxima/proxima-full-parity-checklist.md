...
rather than a closing tick. The scan counts 947 references across ticked boxes at this revision - one fewer than the 948 measured at `ac84d2b`, the difference being the citation replaced above; their own code SHAs and timestamps appear in this Status row, which the scan does not count as a box. The single false record this found - `327d90c` carrying `f8376eb`'s timestamp - was corrected at `5858be5`. |
