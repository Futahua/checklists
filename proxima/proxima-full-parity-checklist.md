...
| Tick audit at this revision | 840 boxes, 789 ticked / 51 open. **Every ticked box names the commit that closed it**: none is unannotated, and the sub-items that carry no SHA close under a parent that does. 869 references resolve against the three local trees by a box-level scan (`D:\Letters\MatTroiSeConMoc\.dsh\audit-checklist-ticks.ps1`, which reads a box across its wrapped continuation lines and distinguishes a closing SHA from one named only as evidence). 0 false timestamps; 164 boxes use the pre-rule style of naming a SHA without a timestamp, which the rule exempts as historical; one reference is unresolvable in every local tree (`6a4b1ff` at line 2905, inside an explanation rather than an annotation, so nothing is claimed by it). The single false record this found - `327d90c` carrying `f8376eb`'s timestamp - was corrected in the revision that carries this row. |
| Next operation | Stage 17's **ten non-test matrix boxes**, made checkable per row the way the three test boxes now are: typed request, runtime validation, typed success, stale/conflict where applicable, validation refusal, storage/recovery failure, request ID, affected entity IDs, observable revision, audit record - each asserted against source in `tests/actionCoverageAudit.test.ts`'s table or recorded in it as not applicable *with the reason*, starting with the Templates row and then the task rows. The eight open work boxes inside the agent-protocol and action sections (result revisions, the UI-versus-agent equivalence rows, intra-batch relation IDs) are the next slice after that; the two recurrence rows stay deliberately open as gaps the audit asserts. **The browser AUTHOR is being sent the packet for this: the evidence on the three boxes ticked at `4d0bfaa` for it to judge, and a request to scope this slice, because the AUTHOR decides what the code means.** Until its ruling lands, the work that does not need it is the standing ritual over both repositories - the tick-reference audit, the required-facts structure check on all four checklists, and a pass over `docs/AUDIT-CHECKLIST.md` and `docs/DECISIONS.md` for rows these slices made stale. Earlier scoping, kept because it is what produced the executor: the Stage 16 executor core and preflight, scoped by the AUTHOR on 2026-09-12 - `executeTemplatePlan` takes a typed plan plus an injected `TaskCreateOperations` port and an injected clock, never imports or receives RecordStore, refuses before the first call on an invalid plan or an untranslatable draft field, reports a mid-batch refusal as partial rather than complete, and is covered by tests for zero calls on failed preflight, the exact emitted requests, deterministic clock and IDs, and a partial write reported as partial. |
...
- [x] UI invocation test exists; — `716739c` @ `2026-09-12T10:26:58+07:00` and `4d0bfaa` @ `2026-09-12T10:29:04+07:00` *(**this one is claimed at the strength it has, not at the strength the box's name suggests.** `src/browser/main.ts` cannot be imported from a test - tests/acceptanceTools.test.ts records why - so no test on this machine clicks `template-execute` and observes the action. What tests/templateExecuteWiring.test.ts does instead is both halves the shell allows: it renders the panel bound to exactly the options the shell passes (`executable: true`, `executing: view.templateExecuting`, `result: view.templateResult`) and checks the rendered running/partial/offered states, then reads the click chain as source - the branch on `action === 'template-execute'`, its reach into `createTemplateTasksAction()`, and the refusal-versus-clean-run split in the state it sets. The audit row asserts the chain reaches `createTemplateTasksAction(` and that this file names the verb, so a later rename cannot leave the claim standing on nothing. A synthetic click through the real chain would need a second harness that does not exist; inventing one is a structural decision, not this box.)*
- [x] equivalence test exists. — `4d0bfaa` @ `2026-09-12T10:29:04+07:00` *(D69's ruling is what this box is judged by: records, not request lists. tests/templateEquivalence.test.ts builds the same three tasks twice in two isolated stores - once through the ordinary `createTask` path, once through `executeTemplateAction` - and compares the decoded canonical records field by field with `id`, `createdAt` and the revisions normalised away, plus a control that would notice a difference the normalisation does not hide. It drove `executeTemplatePlan` until `4d0bfaa`, which compared the manual path against a function both shipped callers bypass; the comparison now runs through the same shipped entry the panel and an agent submission call. The executor keeps its own coverage in tests/templateExecution.test.ts.)*
