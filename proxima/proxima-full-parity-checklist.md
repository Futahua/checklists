...
| Suite at `e75d76a` | fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under **default parallelism**, 244 test files / 1606 tests, run 2026-09-12 in the working tree above (243 / 1601 at `2db9fbd`, 242 / 1595 at `8b4e093`, 241 / 1591 at `b34ef53`, 241 / 1590 at `f9d7c63`, 241 / 1589 at `dc5f908`, 241 / 1588 at `70f5549`, 241 / 1587 at `4566bd6`, 241 / 1585 at `5781e96`, 241 / 1582 at `2f90e2c`, 241 / 1581 at `b980c33`, 240 / 1578 at `65f88ef`, 227 / 1492 at `4d0bfaa`). The file count is complete coverage rather than a partial collection - 244 `*.test.ts` files exist in the tree and the runner collects all 244 - which is a check worth keeping because the As-you-Go repository turned out to have three test files no script ran |
| Tick audit at this revision | 840 boxes, **822 ticked / 18 open** at `9d4ebb1`, or 731 / 17 counting only top-level boxes. The last three slices each closed at least one box, and this one closed four: the ten "for every row above" umbrellas are measured columns, and the measurement at `9d4ebb1` (315 satisfied / 44 gaps / one n/a of 360 cells) is what closed four of them - two of those gaps had been in the tests rather than in the code. **Every ticked box names the commit that closed it**: none is unannotated, and the sub-items that carry no SHA close under a parent that does. 926 references resolve against the four local trees by a box-level scan (`D:\Letters\MatTroiSeConMoc\.dsh\audit-checklist-ticks.ps1`, which reads a box across its wrapped continuation lines and distinguishes a closing SHA from one named only as evidence). 0 false timestamps; 164 boxes use the pre-rule style of naming a SHA without a timestamp, which the rule exempts as historical; one reference is unresolvable in every local tree (`6a4b1ff`, inside an explanation rather than an annotation, so nothing is claimed by it). The scan counts 927 references across ticked boxes at `9d4ebb1`; their own code SHAs and timestamps appear in this Status row, which the scan does not count as a box. The single false record this found - `327d90c` carrying `f8376eb`'s timestamp - was corrected at `5858be5`. |
...
- [ ] runtime validation exists; *(33 of the 36 cells are gaps, and the Templates row is a gap too: the record layer accepts typed requests and validates inside itself, so a per-row runtime-validation boundary is the one column this agenda asks for that the architecture deliberately does not have. The three satisfied cells are the schema rows, whose `src/app/propertySchemaActions.ts` boundary parses `unknown`.)*
...
- [ ] typed stale/conflict where applicable; *(clean on all 36 cells of the five families and **still open**, because "for every row above" includes the Templates row and that row's own table records a real gap here: a semantic conflict arriving from the port is reported as `creation-refused`, so nothing in the result distinguishes it from an ordinary refusal. This box closes when that result does, not when the count does.)*
...
- [ ] typed storage/recovery failure exists; *(clean on all 36 cells and open for the same reason as the conflict row above: the Templates row collapses a storage failure into `creation-refused`, and recovery is not applicable on that path because a create goes through `createIfAbsent` and never uses the recovery coordinator.)*
- [ ] request ID exists; *(31 satisfied / **5 gaps**, and all five are the task family's: the record layer accepts typed requests and drops the coordinator's id, while every other family now has a semantic boundary that mints one. The Templates row is satisfied.)*
...
- [ ] event/audit record exists; *(31 satisfied / **5 gaps**, the same five task-family rows: the record layer emits no event of its own, and the terminal event comes from the semantic boundary. The Templates row is satisfied through the semantic sink.)*
...
- [ ] Every such action has typed success/refusal/stale/failure. *(One of the three gaps this box named is closed and two remain, which is why it is still open. Closed at `9d4ebb1`: the schema field-edit row's post-action state is now read back out of the store at the revision the write reported rather than taken from the value the operation returned. Still open: the Templates row collapses a semantic conflict **and** a storage failure into `creation-refused`, and `project.delete` answers with counts where its siblings answer with ids. Each is a cell in `tests/actionCoverageAudit.test.ts` with its own reason, and a family-wide "yes" here would contradict them - which is the difference between this box and the four "for every row above" umbrellas that closed at the same commit: those are columns, and their cells are all satisfied; this box asks the sharper question of whether each kind of failure is distinguishable on each row.)*
