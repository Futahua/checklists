...
| Suite at `2f90e2c` | fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under **default parallelism**, 241 test files / 1582 tests, run 2026-09-12 in the working tree above (241 / 1581 at `b980c33`, 240 / 1578 at `65f88ef`, 227 / 1492 at `4d0bfaa`). The file count is complete coverage rather than a partial collection - 241 `*.test.ts` files exist in the tree and the runner collects all 241 - which is a check worth keeping because the As-you-Go repository turned out to have three test files no script ran |
...
| Next operation | **Four slices are in: the agent write path (`32132bb`), the property-schema action layer (`65f88ef`), scoped ordering asserted across surfaces (`b980c33`) and the schema family routed through the agent wire (`2f90e2c`).** Twenty-four boxes are left, and the largest group - the agent reach of the remaining families - is now two families deep with a stated rule rather than a list: a family's verbs enter `src/app/agentWritePath.ts` when its operation runs with no cockpit in the dependency set, and a trusted layer that already minted an id hands it down so one submission stays one run. What qualifies next is anything whose operation can be made read-model-free; what does not is every verb that finds its record in a rendered projection (`task.create`, `task.timeline.change`, the `project.*`, `event.*`, `workflow.stage.*` and bulk verbs). Behind that: the operation-only rows' callers (task recurrence, the event recurrence set/clear), the schema family's missing UI caller, and the boxes that are not about code (panel sizing, the navigator-render extraction behind the project-archive box, the open questions, and the scope statements). The contract is not to be redesigned: `src/app/semanticAudit.ts`, one id minted at each boundary, one terminal event after convergence, and `ProximaActionDispatcher.auditSemantic` as the shell's sink. |
