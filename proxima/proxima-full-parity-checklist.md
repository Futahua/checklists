...
| Suite at `4d0bfaa` | fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under **default parallelism**, 227 test files / 1492 tests, run 2026-09-12 in the working tree above (the earlier figures on this row were 220 / 1456 at `c2ef9f1`, re-run as 223 / 1468 at `c689283`). The file count is complete coverage rather than a partial collection - 227 `*.test.ts` files exist in the tree and the runner collects all 227 - which is a check worth keeping because the As-you-Go repository turned out to have three test files no script ran |
| Tick audit at this revision | 840 boxes, 789 ticked / 51 open. **Every ticked box names the commit that closed it**: none is unannotated, and the sub-items that carry no SHA close under a parent that does. 871 references resolve against the four local trees by a box-level scan (`D:\Letters\MatTroiSeConMoc\.dsh\audit-checklist-ticks.ps1`, which reads a box across its wrapped continuation lines and distinguishes a closing SHA from one named only as evidence). 0 false timestamps; 164 boxes use the pre-rule style of naming a SHA without a timestamp, which the rule exempts as historical; one reference is unresolvable in every local tree (`6a4b1ff`, inside an explanation rather than an annotation, so nothing is claimed by it). The single false record this found - `327d90c` carrying `f8376eb`'s timestamp - was corrected at `5858be5`. The same tool run against `papers\quick-run.md` found two empty annotations a tick script had dropped, restored there from the commits that added the cases. |
| Semantic envelope at `4b241f8` | **Fourteen rows are wired**: the nine task rows (`e0d097e`, `a7bf33e`, `c81bc55`, `99c13f5`) and the five Schedule verbs - create, update, delete, reschedule, resize - which all run through one sequence, so `runEventWrite` mints the id once and the journal names the verb as `event.<verb>` (`4b241f8`). Each action returns the id on every result and appends exactly one terminal event after convergence through the shell's sink. Three shapes were decided by the work rather than assumed: the drop wrapper mints once and hands its id down so its own refusals are journalled and one drop still leaves one event; a bulk run is one operation whose single event's outcome comes from its members; and a create in either family names **no** target on a refusal, because it has no id yet and guessing one would be a lie. Still gaps: the two recurrence rows (`eventRecurrenceActions.ts`) and the project, workflow-stage and schema families. | `src/app/semanticAudit.ts` holds the outcome vocabulary - accepted / rejected / **partial**, which is neither of the other two - the one bounded event a run owes, the sink it goes through, and the single rule that decides the outcome from what a run wrote and whether it stopped. `executeTemplateAction` mints a semantic request id at the boundary before anything can refuse, returns it on both branches, and appends exactly one terminal event **after convergence**, so the event can carry the state the surfaces actually reached. `ProximaActionDispatcher.auditSemantic` maps the three outcomes onto the ring's own kinds and categories (acceptance domain, zero-write refusal diagnostic, partial domain) and stamps the dispatcher's **real** state revision rather than one the record layer invented. The id is deliberately not the coordinator's - that one exists only once an update or delete reaches the coordinator, and one shared id would give a batch one recovery identity - the record layer is untouched, and no caller can supply an id, because the request type lost the field it never read. Evidence so far: the template family (`tests/templateSemanticAudit.test.ts`, 7 cases), the dispatcher sink case (`tests/actionProtocol.test.ts`), task create (`tests/taskCreateSemanticAudit.test.ts`, 7 cases) and the editor's save/delete (`tests/taskEditorSemanticAudit.test.ts`, 7 cases) - each mutation-tested by changing the event's action type and watching it fail, so the files observe the actions rather than themselves. The matrix now records coverage per row, so the rows still carrying a gap keep asserting its absence. |
| Stage 17 matrix at `f9c202a` | **Complete for every row the agenda carries**: 36 rows / 360 cells - 248 satisfied, 111 gaps, one not applicable at that revision - in `tests/actionCoverageAudit.test.ts`, one table per family (task, event, project, workflow-stage, schema) with each family's shared cells written once and each row's request, refusal, ids and observable cells written for that row. Each cell is satisfied, a gap, or not applicable **with its reason**; a gap is asserted as an absence with a witness beside it and its anchor asserted before it scopes anything, so a gap fails the day it closes and no check can pass by slicing the last character of a file it never read. Five walkers assert the ten columns in the AUTHOR's order and that each table covers exactly the rows the file already carries. What remains is not scattered: 35 rows still have no untrusted boundary accepting `unknown` (the only one in the tree is the template submission entry), 35 still carry no semantic request id, 35 still emit no semantic event, the retained task recurrence is unwired **and** unobserved, the schema field edit is asserted on the record the operation returned and never read back, and `project.delete` answers with counts rather than the ids it would affect. **Templates row after `ac6115a`**: seven satisfied, three gaps, with stale N/A for a create-only run and recovery N/A because `createIfAbsent` never journals. |
| Next operation | **The two recurrence rows are next, at `4b241f8` @ `2026-09-12T11:32:02+07:00`**, because they are the event family's last two and they live in a different module: `eventRecurrenceActions.ts` holds `updateOccurrenceAction` and the series-scoped path, which write through the same record layer but not through `runEventWrite`. They need the envelope in their own sequence, with the scope in the journal's verb (`event.occurrence.change` for one occurrence, `event.series.change` for the series) so a reader can tell which one a selection meant. Then the project rows - including the refusal-only delete, whose success branch is deliberately absent and whose cell is the one `n/a` in the matrix - then the workflow-stage and schema rows. Each is mechanical: deps gain `ids` and `audit`, the result carries `requestId`, the terminal event names the semantic verb and the ids the run touched, the tests that build those deps take the shared recording sink from `tests/test-semantic-audit.ts`, and the row's two cells flip through `overrides` so its unwired siblings keep asserting the gap. The record-layer contract stays untouched, internal child writes keep their own coordinator ids, and no wire accepts a caller-supplied id. After the wiring: the eight open work boxes inside the agent-protocol and action sections - including the finding the matrix surfaced, that the dispatcher refuses every record verb with `action-not-available` before the record layer, which is HARD GATE E's question in its sharpest form - and the ten non-test matrix boxes whose per-column meaning the AUTHOR fixed. The two recurrence rows of the *record layer* stay deliberately open as gaps the audit asserts. The contract exists and is not to be redesigned: `src/app/semanticAudit.ts` (outcome vocabulary including `partial`, the one bounded event, the sink, and the single outcome rule), the id minted at each action's boundary and returned on every branch, the terminal event appended after convergence, and `ProximaActionDispatcher.auditSemantic` as the sink every shell composition supplies. What each remaining family needs is mechanical but not small: its deps gain `ids` and `audit`, its result carries `requestId`, every test that builds those deps supplies the two pieces (`tests/test-semantic-audit.ts` has the recording sink and the id generator so no test invents its own), and the family's three matrix cells flip from gap to satisfied with behavioural evidence. Task rows first (they are the most-used and the bulk report is the awkward one, since a selection reports one outcome per member), then event, project, workflow-stage and schema. The record-layer contract stays untouched throughout, internal child writes keep their own coordinator ids, and no wire accepts a caller-supplied id. After that, the eight open work boxes inside the agent-protocol and action sections - including the finding the matrix surfaced, that the dispatcher refuses every record verb with `action-not-available` before the record layer, which is HARD GATE E's question in its sharpest form - and the two recurrence rows stay deliberately open as gaps the audit asserts. Earlier scoping, kept because it is where the contract came from: a semantic request id at the public semantic-action boundary, before validation and before the write path is resolved, because the coordinator's id cannot serve that purpose; the record layer left alone; the event appended through an injected audit sink after convergence with the real state revision, never a fabricated one; and `partial` represented explicitly, because "one record landed and the second failed" is neither an acceptance nor a rejection. Kept because it is still the scope order: **a separate semantic request id at the public semantic-action boundary** - before validation and before the write path is resolved - and returns it on every success, refusal and partial result, because the coordinator's id cannot serve that purpose: it exists only once an update or delete reaches the coordinator, while the semantic id must also cover refusals decided before storage, creates that go through `createIfAbsent`, a `writes-unavailable` outcome, and one `template.execute` spanning several child creations - and reusing one coordinator id for a batch would give several underlying mutations the same recovery identity. The record-layer contract is left alone, internal child writes keep their own coordinator ids, and **no external caller may supply the semantic id** (the wire gets no requestId; trusted internal layers may pass the already-minted value down). The event half reuses the bounded `EventRing` and its existing action/entity/request correlation vocabulary while preserving its categories: a clean acceptance is `action.accepted`/`domain` and a zero-write refusal is `action.rejected`/`diagnostic`, while **partial needs explicit representation** - `action.partial` or an equivalent explicit outcome field carrying every landed entity id plus the refusal code, because "one record landed and the second failed" is neither an acceptance nor a rejection. The event is appended **through an injected audit sink after convergence**, where composition can supply the real observable state revision; the record layer must not fabricate one, and if no real revision is available that matrix cell stays a gap rather than using zero. The AUTHOR called this a semantic request envelope plus a semantic terminal event sink, **not a redesign of `recordMutation.ts`**, and was explicit that these boxes must not wait for an unrelated audit. After it, the cells it flips are re-recorded rather than deleted, and the eight open work boxes inside the agent-protocol and action sections follow - including the finding this matrix surfaced, that the dispatcher refuses every record verb with `action-not-available` before the record layer, which is the HARD GATE E question in its sharpest form. The two recurrence rows stay deliberately open as gaps the audit asserts, which the AUTHOR confirmed. Earlier scoping, kept because it is what produced the executor: the Stage 16 executor core and preflight - `executeTemplatePlan` takes a typed plan plus an injected `TaskCreateOperations` port and an injected clock, never imports or receives RecordStore, refuses before the first call on an invalid plan or an untranslatable draft field, reports a mid-batch refusal as partial rather than complete, and is covered by tests for zero calls on failed preflight, the exact emitted requests, deterministic clock and IDs, and a partial write reported as partial. |
