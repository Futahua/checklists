...
- [ ] UI invocation test exists; — **reopened 2026-09-12 by the browser AUTHOR, on evidence that had ticked it.** `716739c` @ `2026-09-12T10:26:58+07:00` and `4d0bfaa` @ `2026-09-12T10:29:04+07:00` produced the two halves this repository can currently produce - the panel rendered while bound to the shell's own options, and the click chain read as source - and the AUTHOR rejected that as this box's evidence: *"reading main.ts as text cannot prove the listener actually invokes the action under runtime composition."* What closes it is named rather than left to taste: extract the template execution binding from the unimportable `src/browser/main.ts` into an importable `src/browser/templateExecuteBinding.ts`, have `main.ts` use it, then render the real panel, bind that module, click the stable key the renderer already supplies (`data-c1-key="template-execute"`) through the **existing** `InteractionHarness`, and observe `executeTemplateAction` together with the running/result state. The AUTHOR's instruction is explicit that this is not a reason to build a second general harness.
- [ ] equivalence test exists. — **reopened 2026-09-12 by the browser AUTHOR.** D71's boundary was ratified - moving the comparison from `executeTemplatePlan` up to `executeTemplateAction` at `4d0bfaa` @ `2026-09-12T10:29:04+07:00` is correct, because it proves record equivalence through the shipped action layer - but that case answers Stage 16's box, not this one. This box sits beside *agent invocation* and *UI invocation*, so its claim is equivalence **between those two callers**: once the real UI chain is testable, two isolated worlds - a UI click versus `submitTemplateExecution`, same template and project - compared on outcome and partial semantics plus the normalised resulting records. The agent entry already reaches the same action, so what is being compared is callers rather than layers.
