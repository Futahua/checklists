...
creator walking `# Creator acceptance walk` at the foot of this file, which no automated evidence replaces. **That route was located and its shape checked rather than assumed** (this pass, with nothing launched and the creator's own running Papers untouched). The packaged app is `D:\Letters\MatTroiSeConMoc\PAPERS 3\Papers-3\release\win-unpacked\Papers.exe`, and the precedent for driving it is `probes\015r3-live-proof\cdp.mjs`, which spawns that binary with `--remote-debugging-port` and talks CDP to the real surface; `probes\016r` is the precedent for a *disposable* instance, launched with its own `--user-data-dir`, which is what keeps a probe off the creator's instance - and `run-016-phase-a-eye-check.mjs` says in its own text that their real Papers must not be closed. **What that means for the two Performance boxes is the sharp part, and it is new rather than a restatement:** a disposable profile starts with an empty workspace, while the gate is 10k-20k *real* rows with graph mode active, so the corpus those boxes name is the creator's vault. The number therefore either comes from their live instance with their consent, or from a disposable one seeded with a synthetic corpus of that size - and that second number would be evidence for a different claim, because the box is about the frame-latency budget over a real corpus. No automated evidence replaces `# Creator acceptance walk` either. Recorded so the next session does not re-derive it: the app, the launch pattern, the profile discipline and the corpus question are all known now, and the decision that remains is whose instance the number comes from.
